...
def registration_departments(request):
    return render(request, "registration_departments.html", {"departments": Department.objects.all()})
...
def forgot_password(request):
    if request.method == "POST":
        ...
        if user:
            ...
            messages.error(request, "No employee account was found with that email address.")
...
def progress_updates(request):
    user = current_employee(request)
    if not user: return redirect("login")
    return render(request, "employee/progress_updates.html", {"updates": user.progress_updates.select_related("task").all()})
...
def extension_requests_employee(request):
    user = current_employee(request)
    if not user: return redirect("login")
    return render(request, "employee/extension_requests.html", {"requests": user.extension_requests.all()})
